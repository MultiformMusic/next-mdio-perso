...
export async function libGetPorfolios(preview = false, offset = 0) {

    const client = getClient(preview);
    const results = await client.fetch(`*[_type == "portfolios" && visibility == true] | order(order asc) { ${portfoliosFields}}[${offset}...${offset + 6}]`);

    return results;
}

export async function libGetAllPorfolios(preview = false, offset = 0) {

    const client = getClient(preview);
    const results = await client.fetch(`*[_type == "portfolios" && visibility == true] | order(order asc) { ${portfoliosFields}}`);

    return results;
}

export async function libGetPorfoliosCount(preview = false) {

    const client = getClient(preview);
    const results = await client.fetch(`*[_type == "portfolios" && visibility == true] | order(order asc) { "portfoliosCount": count(portfolios)}`);
